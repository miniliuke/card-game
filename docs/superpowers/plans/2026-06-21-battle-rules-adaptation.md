# Battle / Game 适配规则层 — 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 删除旧 `game.rs` demo，重写 `battle.rs` 使其通过 `apply_action` / `resume` 驱动 `src/rules/` 规则层，完整覆盖 6 动作 + 弃牌 + 选贵族 + 终局，事件驱动动画。

**Architecture:** `BattleModel(GameState)` 入资源；玩家意图经 `ActionQueue` → `apply_actions` 调 `apply_action`/`resume` → 产物 `Vec<GameEvent>` 入 `PendingEvents` + 挂起态入 `PendingPhase`；`play_events` 逐个消费事件播放飞牌/发牌动画；`commit_pending_phase` 在动画空闲后把 `PendingPhase` 提交为 `BattlePhase`（`Idle`/`AwaitDiscard`/`AwaitNobleChoice`/`GameOver`），弹对应覆盖层。

**Tech Stack:** Rust 1.90 (edition 2024), Bevy 0.18, 已有 `rand = "0.9"`. UI 用 Bevy 原生 `Node`/`Button`/`Interaction`. `percent(n)` / `px(n)` 是 Bevy `ui::geometry` 自由函数（经 `use bevy::prelude::*` 可用），返回 `Val`.

**Spec:** `docs/superpowers/specs/2026-06-21-battle-rules-adaptation-design.md`

**重要约定:**
- 这是 git 仓库（branch `master`）。每个 Task 末尾 commit。
- Bevy UI spawn 代码无 headless 测试手段 → 对可纯测的逻辑（动作映射、门控、状态机、颜色映射）写 `#[cfg(test)]` 单测；对 UI spawn 系统以 `cargo build` + `cargo run` 手动验证收尾。
- 规则层 `src/rules/` **本计划不改**，只 `use crate::rules::*`.
- `BattleAction` 名称与 `rules::PlayerAction` 变体一一对应，但 `BattleAction` 是 Bevy `Component`（`#[derive(Component, Clone, Copy)]`），需注意 `Vec<GemColor>` 不能 `Copy` → 用 `Box<[GemColor;3]>` 或单独标记组件（见 Task 3）。
- 现有 `battle.rs` 的调色板常量（`INK`/`PANEL`/`GOLD` 等）、`spawn_ambient_shapes`、`gem_color`/`color_name`/`color_short`、`FlyAnimation`/`DealAnimation`、`UiDirty`/`FocusCursor`/`AnimationCounts`、`responsive_battle_layout` 模式全部保留复用。
...
| 文件 | 操作 | 职责 |
|---|---|---|
| `src/game.rs` | **删除** | 旧 demo 引擎，被 `rules/` 取代 |
| `src/main.rs` | 修改 | 移除 `mod game;`；`AppState` 不变 |
| `src/battle.rs` | **完全重写** | 全部 battle UI + 输入 + 事件动画 + 状态机 |

`battle.rs` 内部分区（用 `// === SECTION ===` 注释分隔，沿用现有风格）：

1. 常量与调色板
2. 资源（`BattleModel`/`PendingEvents`/`BattlePhase`/`PendingPhase`/`TokenPicker`/`DiscardBuffer`/`PendingNobleCandidates`/`ActionQueue`/`FocusCursor`/`AnimationCounts`/`UiDirty`/`TurnCount`）
3. 组件（marker + 文本 marker + 输入标记）
4. `BattleAction` / `FocusZone`
5. `setup_battle` / spawn 函数
6. 输入系统（mouse/keyboard/3 个 overlay input）
7. 核心（`apply_actions`/`play_events`/`commit_pending_phase`）
8. 动画（`animate_flights`/`animate_deals`）
9. 刷新（`refresh_battle_ui` 等）
10. 辅助（`gem_color` 等，含 6 色）
11. `#[cfg(test)] mod tests`

---

## Task 1: 删除 game.rs，battle.rs 改用 rules，建立最小可编译骨架

**目标：** 移除旧依赖，让 `battle.rs` 重新编译（仅骨架，无功能），保证后续每个 Task 都在可编译基础上增量。

**Files:**
- Delete: `src/game.rs`
- Modify: `src/main.rs:8-9`（移除 `mod game;`）
- Modify: `src/battle.rs`（整体替换为骨架）

- [ ] **Step 1: 删除 game.rs**

Run: `git rm src/game.rs`

- [ ] **Step 2: main.rs 移除 mod game**

修改 `src/main.rs`，删除这一行：

```rust
mod game;
```

确认 `mod battle;` 与 `mod rules;` 保留。

- [ ] **Step 3: 用骨架整体替换 battle.rs**

把 `src/battle.rs` 整体替换为以下骨架（保留调色板、动画组件、辅助函数；移除所有引用 `game::*` 的代码；`BattlePlugin` 注册一个空系统占位以保证编译）：
...
Expected: 编译通过（可能有 `unused` 警告，本阶段忽略；后续 Task 消费这些项）。

- [ ] **Step 5: 跑测试**

Run: `cargo test battle --message-format=short`
Expected: 2 个测试通过（`gem_color_handles_all_six`、`color_name_covers_gold`）。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "refactor(battle): drop game.rs demo, skeleton battle on rules"
```

---

## Task 2: 资源与状态机骨架（纯逻辑 + 单测）

**目标：** 定义核心资源与状态机类型，写 `PendingPhase` 提交时机的纯逻辑 + 单测。这是整个适配的中枢。

**Files:**
- Modify: `src/battle.rs`（在 `AnimationCounts` 之后插入资源定义；在文件末尾 tests 模块加测试）

- [ ] **Step 1: 写失败测试 — phase 提交时机**
...
Run: `cargo test battle --message-format=short 2>&1 | findstr /C:"error" /C:"FAILED"`
Expected: FAIL — `PendingEvents`/`BattlePhase`/`PendingPhase`/`should_commit_phase` 未定义。

- [ ] **Step 3: 定义资源与状态机类型**

在 `AnimationCounts` impl 块之后插入：
...
Run: `cargo test battle --message-format=short`
Expected: 全部测试通过（含 Task 1 的 2 个 + 本 Task 的 4 个）。

- [ ] **Step 5: Commit**

```bash
git add -A
git commit -m "feat(battle): add phase machine resources and commit predicate"
```

---

## Task 3: BattleAction 与 PlayerAction 映射（纯逻辑 + 单测）

**目标：** 定义 `BattleAction`（Bevy 组件）与 `BattleAction::to_player_action` 映射，确保与规则层一一对应。
...
Run: `cargo test battle --message-format=short 2>&1 | findstr /C:"error"`
Expected: FAIL — `BattleAction`/`Triple`/`to_player_action` 未定义。

- [ ] **Step 3: 定义 BattleAction 与映射**

在 `BattleScreen` 组件定义之前插入：
...
```bash
git add -A
git commit -m "feat(battle): BattleAction <-> PlayerAction mapping"
```

---

## Task 4: 输入门控谓词（纯逻辑 + 单测）

**目标：** 定义 `can_act` 门控（`Idle && !busy && events empty`），后续 mouse/keyboard/apply_actions 系统用它做 `run_if`。
...
Run: `cargo test battle --message-format=short 2>&1 | findstr /C:"error"`
Expected: FAIL — `can_act` 未定义。

- [ ] **Step 3: 实现 can_act**

在 `should_commit_phase` 之后插入：
...
```bash
git add -A
git commit -m "feat(battle): can_act input gate predicate"
```

---

## Task 5: setup_battle 接入 GameState，注册全部资源

**目标：** `OnEnter(Battle)` 时用 `GameState::new_seeded(2, seed)` 初始化，插入所有资源，spawn battle 屏幕 root（空 UI，后续 Task 填内容）。本 Task 是 UI 起点，以 `cargo build` + `cargo run` 验证（手动看到黑屏进 battle 不 panic 即可）。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 改写 setup_battle 与 cleanup_battle，注册资源**

替换 Task 1 占位的 `setup_battle` 和 `cleanup_battle`，并在 `BattlePlugin::build` 注册全部资源/系统：
...
新增资源/组件占位（放在 `PendingNobleCandidates` 之后）：
...
Run: `cargo build --message-format=short`
Expected: 通过（可能有 unused 警告，后续 Task 消费）。

- [ ] **Step 3: 手动验证（可选，需图形环境）**

Run: `cargo run`
Expected: 主菜单点 NEW ADVENTURE → 进入黑色 battle 屏幕（空 UI），不 panic；ESC 应无反应（键盘系统未接，下个 Task 加）。若无法运行图形环境，跳过此步。

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "feat(battle): wire GameState + resources in setup_battle"
```

---

## Task 6: spawn UI — 玩家面板（2 人 rich 布局）

**目标：** 复用并扩展旧 `spawn_player_panel`，加入 score/bonus/tokens/gold/reserved/nobles 行。本 Task 只 spawn 静态结构 + marker 组件，文本内容由 Task 11 的 `refresh_battle_ui` 填充。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 定义面板相关 marker 组件**
...
- [ ] **Step 3: 在 setup_battle 的 root 下挂两个玩家面板**

在 setup_battle 的 `.spawn((... BattleRoot))` 后接 `.with_children(|root| { spawn_player_panel(root, 0); spawn_player_panel(root, 1); })`（暂不接市场，下个 Task 加）。完整改 root spawn：
...
Run: `cargo run`
Expected: 进 battle 后左右两个玩家面板可见（文本为占位 "0 PTS" 等），不 panic。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): spawn 2-player panels with score/token/reserved/nobles rows"
```

---

## Task 7: spawn UI — 市场（3 行 × 4 槽 + deck 盲抽按钮 + R 保留按钮）

**目标：** spawn 市场，每张可见卡是 Buy 按钮 + 叠加的 R 保留按钮；每行左侧 deck 计数块是 ReserveDeckCard 按钮。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 定义市场 marker 组件**
...
- [ ] **Step 2: 实现 spawn_market + spawn_market_row + 卡面**
...
- [ ] **Step 3: setup_battle 在两面板之间插入市场**

`setup_battle` 在 Task 5/7 已局部构造 `let model = BattleModel(state);`（末尾才 `insert_resource`），故 `spawn_market(root, &model)` 直接可用。修改 root 的 `.with_children`，把市场放在两个玩家面板之间：
...
Run: `cargo run`
Expected: battle 中间出现 3 行市场，每行 4 张卡 + 左侧 deck 计数块，卡面显示费用点。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): spawn market with buy/reserve/deck buttons"
```

---

## Task 8: spawn UI — Token supply（5 色 + ×2 徽章 + Confirm/Clear + 金色信息）

**目标：** spawn 筹码供应区。普通色按钮带 `BattleAction`（但 TakeThree 走缓冲，故按钮只挂 `SupplyButton` marker 不直接挂 action；Confirm 才入队）。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 定义 supply marker 组件**

在 `ReserveMarketButton` 之后插入：
...
- [ ] **Step 2: 实现 spawn_token_supply**

在 spawn_market_row 之后插入。注意：`spawn_market` 末尾要调用它；本步同时改 spawn_market 末尾加 `spawn_token_supply(market, model);`：
...
在 `spawn_market` 函数体末尾（`for level` 循环之后、闭合 `})` 之前）加：

```rust
            spawn_token_supply(market, model);
```

并在 spawn_market 外、market 容器之下的位置加 Selection HUD。改 spawn_market 末尾为：
...
```rust
            ...
            spawn_token_supply(market, model);
            // Selection HUD (Confirm/Clear + counter)
            market
                .spawn(Node {
                    width: percent(100),
                    align_items: AlignItems::Center,
...
Run: `cargo run`
Expected: 市场下方出现 5 色筹码按钮（数量≥4 的有 x2 徽章）+ 金色信息 + TAKE 3 / CLEAR 按钮。

- [ ] **Step 5: Commit**

```bash
git add -A
git commit -m "feat(battle): spawn token supply with x2 badges and selection HUD"
```

---

## Task 9: spawn UI — top bar / footer / 公共贵族区 + 焦点组件桥接

**目标：** 补 top bar（回合数 + FINAL 标记）、footer（操作提示 + status）、公共贵族区（可见贵族徽章，动画源）。把 `Focusable` 组件挂到各按钮上供键盘系统（Task 13）使用。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 定义剩余 marker 组件**

在 `SelectionHudText` 之后插入：
...
- [ ] **Step 2: 实现 spawn_top_bar / spawn_footer / spawn_noble_board / spawn_ambient_shapes**
...
- [ ] **Step 3: setup_battle root 组装全部区块**

改 root `.with_children`：
...
注意：`spawn_noble_board` 需 `&model`，而 model 在 setup 末尾才 insert_resource。保持 Task 7 改法（局部 `model` 变量先构造）。
...
Run: `cargo run`
Expected: 完整布局可见——顶栏、贵族区（3 个徽章）、左右玩家面板、中央市场、底栏。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): spawn top bar, noble board, footer; assemble layout"
```

---

## Task 10: 鼠标输入 — supply 选择缓冲 + Confirm/Clear + market/deck/reserve 点击

**目标：** `mouse_actions` 处理：supply 单击加入 `TokenPicker`；x2 单击直接入队 `TakeTwoSameTokens`；Confirm 入队 `TakeThreeDifferentTokens`；Clear 清空；market 卡 / R 按钮 / deck 按钮 / reserved 卡点击入队对应 `BattleAction`。门控 `can_act`。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 注册 mouse_actions 系统（带 run_if）**

改 `BattlePlugin::build`，在 `OnExit` 之后加：
...
Run: `cargo run`
Expected: 点 supply 色 → 无即时反馈（picker 内部变，UI 高亮下个 Task 加）；点 x2 → 触发（但 apply_actions 未实现，暂无效果，下个 Task）；点市场卡 / R / deck → 入队（暂无效果）。

- [ ] **Step 5: Commit**

```bash
git add -A
git commit -m "feat(battle): mouse input for supply buffer, x2, confirm, market actions"
```

---

## Task 11: apply_actions + play_events + commit_pending_phase（核心循环）

**目标：** 消费 `ActionQueue` 调 `apply_action`，事件入 `PendingEvents`，outcome 转 `PendingPhase`；`play_events` 逐个播放（本 Task 先做"消费即置 dirty"，动画细节在 Task 12）；`commit_pending_phase` 在空闲时提交 phase。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 写失败测试 — outcome 转 PendingPhase 映射**
...
Run: `cargo test battle --message-format=short 2>&1 | findstr /C:"error"`
Expected: FAIL — `outcome_to_pending` 未定义。

- [ ] **Step 3: 实现 outcome_to_pending + 事件 GameOver 检测辅助**
...
- [ ] **Step 5: 实现 apply_actions 系统**

插入（注意防重入：若 PendingPhase 有值或忙或事件非空则跳过）：
...
- [ ] **Step 6: 实现 play_events + commit_pending_phase**

插入（本 Task 先实现"逐个消费 + 置 dirty"，动画 spawn 在 Task 12）：
...
- [ ] **Step 7: 注册系统 + 调整 BattlePlugin::build**
...
并把 mouse_actions 的内联闭包门控抽成函数（便于 chain 复用），在文件加：
...
注意 `apply_actions`/`play_events`/`commit_pending_phase` 不加 `run_if`（它们内部各自防重入/空检查）。
...
Run: `cargo run`
Expected: 点 supply 3 色 → TAKE 3 → 玩家筹码变（需 refresh，下个 Task），status 显示"took tokens"；点市场卡若买得起则 status 显示"bought"，否则"Cannot afford"。回合自动推进（规则层）。

- [ ] **Step 10: Commit**

```bash
git add -A
git commit -m "feat(battle): apply_actions + play_events + commit_pending_phase core loop"
```

---

## Task 12: 动画扩展 — 飞牌/金币/贵族/发牌（事件驱动）

**目标：** 把 Task 11 的"消费即 dirty"升级为真正的动画：`play_events` 按事件 spawn `FlyAnimation`/`DealAnimation` 实体，`animate_flights`/`animate_deals` 推进。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 实现 animate_flights / animate_deals（复用旧逻辑）**
...
- [ ] **Step 2: 升级 play_events — 按事件 spawn 动画**

替换 Task 11 的 `play_events` 为（在 status 更新基础上，对部分事件 spawn fly/deal）：
...
- [ ] **Step 3: 注册动画系统 + 旧 market 卡 buy/reserve 后的处理**

在 `Update` chain 末尾加 `animate_flights`, `animate_deals`：
...
Run: `cargo run`
Expected: 拿筹码/买牌/保留/贵族拜访时看到飞行动画；市场补牌看到发牌动画。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): event-driven fly/deal animations"
```

---

## Task 13: refresh_battle_ui — 全量刷新文本/边框（含 reserved/nobles 动态生成）

**目标：** `refresh_battle_ui` 在 `UiDirty` 时刷新：turn 文本、各玩家 score/state/color/gold、deck 计数、supply 计数、selection HUD、当前玩家面板金边。Reserved 行与 Nobles 行需按规则层现状重建子节点（因卡牌/贵族增减）。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 注册 refresh_battle_ui + responsive_battle_layout + button_hover_effects**
...
- [ ] **Step 2: 实现 refresh_battle_ui（含 reserved/nobles 重建）**
...
fn refresh_battle_ui(
    ...
    }
    {
        if let Ok(mut hud) = texts.p6().single_mut() {
            // selection HUD count — picker not queried here; 留空，下个 step 用独立系统
            **hud = "0/3".to_string();
        }
    }
    {
...
- [ ] **Step 3: 实现 selection HUD 刷新（独立系统，因需 TokenPicker）+ hover/scale/focus/layout**
...
把 `refresh_selection_hud` 加入 `Update` chain（在 `refresh_battle_ui` 后）。
...
Run: `cargo run`
Expected: 顶栏 turn 文本随回合更新；玩家面板 score/筹码数随行动变化；当前玩家面板金边。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): refresh_battle_ui full snapshot + selection HUD"
```

---

## Task 14: 覆盖层 — Discard / Noble / GameOver + overlay 输入 + resume

**目标：** `commit_pending_phase` 在切到非 Idle phase 时 spawn 对应覆盖层；三个 `overlay_input` 系统处理玩家交互 → `resume`。覆盖层在回 Idle/GameOver 时清理。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 定义覆盖层 marker 组件**
...
- [ ] **Step 2: 重写 commit_pending_phase — 切 phase 时 spawn/cleanup 覆盖层**

替换 Task 11 的 `commit_pending_phase`：
...
- [ ] **Step 3: 实现 3 个 overlay_input 系统 + 注册**
...
fn discard_overlay_input(
    ...
    mut model_mut: ResMut<BattleModel>,
    mut pending_events: ResMut<PendingEvents>,
    mut pending_phase: ResMut<PendingPhase>,
    pending_nobles: Res<PendingNobleCandidates>,
    mut turn: ResMut<TurnCount>,
    ...
    if let Ok(interaction) = confirm.single() {
            ...
            && buf.returned.total() == buf.excess
        {
            ...
            match resume(&mut model_mut.0, pid, Resume::DiscardTokens(returned)) {
                Ok(r) => {
...
把三个系统加入 `Update` chain（在 `mouse_actions` 后、`apply_actions` 前）：
...
Run: `cargo run`
Expected:
- 拿筹码超过 10 → 弹 Discard overlay → 点色归还 → CONFIRM → 回合推进。
- 买牌满足多贵族 → 弹 Noble overlay → 选一个 → 回合推进。
- 达 15 分 → final round → 最终弹 GameOver overlay → BACK TO MENU 回菜单。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): discard/noble/gameover overlays + resume input"
```

---

## Task 15: 键盘输入（焦点 zone 导航）+ ESC 返回菜单 + supply 选中高亮

**目标：** `keyboard_actions` 实现方向键区域内移动、Tab 跨区、Enter 激活、ESC 返回菜单。Supply 选中态在 `update_focus_visuals`/独立系统里高亮。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 给关键按钮挂 Focusable 组件**

修改 `spawn_card_button`、`spawn_supply_button`、deck 按钮、Confirm/Clear，在 spawn 元组里加 `Focusable { zone, normal_border }`。示例（市场卡，改 BattleAction 那行附近）：

在 `spawn_card_button` 的 spawn 元组里，`BattleAction::BuyVisibleCard { level, idx: slot }` 之后加：
...
deck 按钮（spawn_market_row 的 deck 块）加：
...
ConfirmTake3 / ClearSelection 各加 `Focusable { zone: FocusZone::ConfirmTake3, normal_border: GOLD.with_alpha(0.5) }` / `FocusZone::ClearSelection`。

- [ ] **Step 2: 实现 keyboard_actions**
...
把 `keyboard_actions.run_if(input_gate)` 加入 `Update` chain（mouse_actions 后）。

- [ ] **Step 3: supply 选中高亮 — 改 update_focus_visuals 同时处理 picker**

新增独立系统 `highlight_selected_supply`：
...
Run: `cargo run`
Expected: 方向键移动市场焦点、Tab 切区、Enter 激活、ESC 回菜单、选中的 supply 色金边高亮。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): keyboard focus-zone navigation + ESC + supply highlight"
```

---

## Task 16: Reserved/Nobles 行动态重建 + 3/4 人 compact 布局分支

**目标：** reserved/nobles 行按规则层现状动态 spawn 子节点（买走/保留后更新）；`spawn_player_panels` 按 player_count 分支（2 → 现有 rich；3/4 → 顶部 compact 一排）。

**Files:**
- Modify: `src/battle.rs`

- [ ] **Step 1: 在 refresh_battle_ui 中重建 reserved/nobles 行**

把 Task 13 的 `refresh_battle_ui` 末尾 `let _ = (&mut commands, &reserved_rows, &nobles_rows, &mut turn);` 替换为实际重建逻辑：
...
fn spawn_reserved_card_mini(
    ...
    if is_owner {
        entity = entity
            .insert((Button, ReservedCardButton { player, idx }, BattleAction::BuyReservedCard(idx)));
    }
...
注意：`despawn_descendants` 会移除容器内全部子节点，每帧 dirty 时重建——可接受（dirty 频率低）。但 `ReservedRow`/`NoblesRow` 容器本身保留。

- [ ] **Step 2: 3/4 人 compact 布局分支**
...
改 setup_battle 用 `GameState::new_seeded(PLAYER_COUNT, seed)`，并在 main 容器 spawn 处按 player_count 分支：
...
```rust
            .with_children(|main| {
                if PLAYER_COUNT == 2 {
                    ...
                    spawn_market(main, &model);
                    // 3/4 人：底部一排 compact 卡片（实现见 spawn_compact_panels）
                    spawn_compact_panels(main, &model);
                }
            });
```

新增（compact 仅 score + token 简表 + reserved/noble 计数，reserved 详情仅 active 展开）：
...
Run: `cargo run`
Expected: 保留卡出现在玩家面板 reserved 行；owner 面板可点 Buy；买走后行更新；贵族行显示已获贵族分数。

- [ ] **Step 5: 切 PLAYER_COUNT=4 验证（可选）**

临时改 `const PLAYER_COUNT: usize = 4;`，`cargo run`，确认底部 compact 卡片一排显示，游戏可进行；验证后改回 2。

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat(battle): dynamic reserved/nobles rows + 3/4 compact layout"
```

---

## Task 17: 全量编译 + 测试 + 手动冒烟

**目标：** 确认全绿，跑一遍完整对局（买牌/拿筹码/保留/弃牌/选贵族/终局）。

**Files:**
- Modify: `src/battle.rs`（清理 unused 警告）

- [ ] **Step 1: 全量测试**

Run: `cargo test --message-format=short`
Expected: 全部通过（rules 层测试 + battle 层逻辑测试）。

- [ ] **Step 2: 编译零警告（尽力）**

Run: `cargo build --message-format=short 2>&1 | findstr /C:"warning"`
Expected: 无 `unused`/`dead_code` 严重警告（`allow` 可酌情加）。

- [ ] **Step 3: 手动冒烟对局**

Run: `cargo run`
逐项验证：
- 点 supply 3 色 → TAKE 3 → 玩家筹码 +3，回合推进，飞币动画。
- 点 supply ×2 → 拿 2 同色。
- 点市场卡（买得起）→ 飞牌 + 筹码飞回，分数涨。
- 点市场卡 R → 保留，卡进 reserved 行，+gold（若银行有金）。
- 点 deck BLIND → 盲抽一张进 reserved。
- reserved 卡 Buy（owner 面板）→ 买走。
- 拿筹码超 10 → Discard overlay → 归还 → CONFIRM。
- 满足多贵族 → Noble overlay → 选一个。
- 达 15 → FINAL → 最终 GameOver overlay → BACK TO MENU。

- [ ] **Step 4: Commit（若有清理）**

```bash
git add -A
git commit -m "test(battle): final smoke pass, clean warnings"
```
...
- **Spec 覆盖**：6 动作（Task 3/10/15）✓；保留牌展示（Task 6/16）✓；贵族区（Task 9/14/16）✓；筹码上限弃牌（Task 14）✓；贵族选择（Task 14）✓；15 分终局/最终轮（Task 11/14）✓；事件驱动动画（Task 12）✓；token 选择缓冲（Task 8/10/15）✓；suspend 状态机（Task 2/11/14）✓；2 人 rich + 3/4 compact（Task 16）✓；删除 game.rs（Task 1）✓。
- **占位符扫描**：无 TBD/TODO；UI spawn 代码完整。
- **类型一致性**：`BattleAction`/`Triple`/`to_player_action`（Task 3 定义，10/15 用）✓；`BattlePhase`/`PendingPhase`/`should_commit_phase`/`can_act`/`outcome_to_pending`（Task 2/4/11 定义并互引）✓；`FocusZone`/`FocusCursor`/`Focusable`（Task 5/9/15）✓；marker 组件（Task 6/7/8/9/14）逐 Task 定义后即用 ✓。
